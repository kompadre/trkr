...
int clamped_vel = std::max<int>(0, std::min<int>(127, velocity));
...
int ScaleRate(int midinote, int sensitivity) {
  int x = std::min<int>(31, std::max<int>(0, midinote / 3 - 7));
...
int ScaleCurve(int group, int depth, int curve) {
  ...
  if (curve == 0 || curve == 3) {
    ...
    int raw_exp = exp_scale_data[std::min<int>(group, n_scale_data - 1)];
...
void Dx7Note::compute(int32_t *buf, int32_t lfo_val, int32_t lfo_delay,
  const Controllers *ctrls, int32_t channel_vol) {
  ...
  const FmAlgorithm &alg = algorithms[this->algorithm_];
  ...
  for (int op = 0; op < 6; op++) {
    ...
    if ((alg.ops[op] & OUT_BUS_ADD) && channel_vol < 256) {
      gain = (static_cast<int64_t>(gain) * channel_vol) >> 8;
    }
